<template>
	...
	<div class="antialiased box-border">
		...
		<div class="min-h-screen bg-gray-200 duration-200 dark:bg-groadis-darker">
			<nav
				:class="[
				'fixed right-0  rounded-bl-lg z-50 bg-white dark:bg-groadis-dark border-b shadow border-gray-100 duration-200 dark:border-groadis-dark',
				$store.state.sideBarActive ? 'left-[13.5rem]': 'left-14'
				]"
			>
			...
			<side-bar :links="links" v-if="links.length" :key="route().current()" />
			<!-- Page Content -->
			<main
				:class="[
